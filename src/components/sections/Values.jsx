...
const CARD_ICONS = [
  ...
  <svg key="globe" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  // Book - Discipleship
  <svg key="book" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export function Values() {
  ...
  const staggerRef = useScrollAnimation({ threshold: 0.15 });
    ...
    <SectionWrapper id="values" dark>
      <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <div className="relative overflow-hidden rounded-2xl shadow-2xl">
          ...
          />
          {/* gold accent bar along bottom edge */}
          <div className="absolute bottom-0 left-0 right-0 h-1 bg-gold" />
        </div>

        {/* Right — headline, statement, cards */}
        <div className="flex flex-col justify-center">
          <SectionHeadline dark>{t('values.headline')}</SectionHeadline>

          <p className="mb-10 mt-3 text-lg leading-relaxed text-cream/80 italic">
            {t('values.statement')}
          </p>

          <div ref={staggerRef} className="stagger-children animate-on-scroll flex flex-col gap-5">
            {cards.map(({ key, icon }) => (
